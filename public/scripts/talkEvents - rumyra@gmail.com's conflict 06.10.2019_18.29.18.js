...
Reveal.addEventListener( 'vis_vjing', function(ev) {
  ...
  animateDom = function() {
    ...
    if (data[0] === 144 && data[1] === 5 && data[2] === 0) {
        screenOne.style.filter = 'invert(100%)';
        screenTwo.style.filter = 'invert(100%)';
    } else if (data[0] === 144 && data[1] === 5 && data[2] === 64) {
...
// 24
  ...
  shape
    ...
    .attr("fill", function(d,i) {return "hsla("+Math.round( i*(20+(d/255)) )+",50%,70%,0.8)"})
    .exit().remove();
...
// 24
  ...
  shape
    .attr("r", function(d, i) { return (d)*( (16-i)/6 ); })
